...
- Este Parceiro possui Resposta Imediata para consultas simples ou lotes pequenos, desde que o Produto e Código EAN/GTIN contenha na Base de dados do Parceiro. Todavia, o **Envio de Todos os Produtos** requer prazo de até 45 dias para devolução integral das informações. 
- O Parceiro fornece decisão invidual ao usuário, que pode ou não acatar as alterações tributárias para seus produtos através do próprio Sistema Ganso (quando implementado o recurso).  
- O Parceiro retorna dados suficientes para permitir a criação de Regras Fiscais de Entrada e Saída completas no Sistema Ganso, entretanto, deve ser utilizada a Camada de Tratamento de dados para garantir o direcionamento correto dos dados.
...
Será necessário a criação de Parâmetros de Configuração para o Integrador desta documentação, que requer dados específicos para o correto funcionamento da rotina. Deve ser utilizada a aba **Integrações Fiscais** dos Parâmetros do Sistema, e conter os elementos descritos abaixo;
